...
describe("filesystem", function() {
	...
	it("dotSlash", function(finish) {
		...
		var blob;
		should(function() {
			f = Ti.Filesystem.getFile("./file.txt");
		}).not.throw();
	...
	it("fileStreamResourceFileTest", function(finish) {
		if ("android" === Ti.Platform.osname) {
			should(function() {
				Ti.Filesystem.openStream(Ti.Filesystem.MODE_WRITE, Ti.Filesystem.resourcesDirectory, "stream_test_in.txt");
			}).throw();
			should(function() {
				Ti.Filesystem.openStream(Ti.Filesystem.MODE_APPEND, Ti.Filesystem.resourcesDirectory, "stream_test_in.txt");
			}).throw();
			should(function() {
				var resourceFileStream = Ti.Filesystem.openStream(Ti.Filesystem.MODE_READ, Ti.Filesystem.resourcesDirectory, "stream_test_in.txt");
				resourceFileStream.close();
	...
	//TIMOB-7605
	it("httpClientFileTransfers", function(finish) {
				this.timeout(3e4);
		var count = 0;
		for (var i = 1; 5 >= i; i++) {
